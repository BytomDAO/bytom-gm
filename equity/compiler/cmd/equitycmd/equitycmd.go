...
import (
	...
	"github.com/bytom-gm/equity/compiler"
)
...
func main() {
	...
	imports := map[string]bool{
		...
		"encoding/hex": true,
		"fmt":          true,
		"github.com/bytom-gm/equity/compiler": true,
		"github.com/bytom-gm/protocol/vm":     true,
	}
...
func asGoParams(params []*compiler.Param) (goParams string, imports []string) {
	...
	for _, p := range params {
		...
		case "Asset":
			typ = "bc.AssetId"
			imports = append(imports, "github.com/bytom-gm/protocol/bc")
		case "Boolean":
			typ = "bool"
		case "Hash":
			typ = "[]byte"
		...
		case "PublicKey":
			typ = "sm2.PubKey"
			imports = append(imports, "github.com/bytom-gm/crypto/sm2")
		case "Signature":
			typ = "[]byte"
		case "String":
			typ = "[]byte"
